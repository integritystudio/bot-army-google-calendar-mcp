...
to a caller-supplied `maxResults` (every accidental truncation is now migrated away, but
the trap remains for new call sites); a filter can add only one
...
- `list-unread-emails.mjs [--stats] [--total]` — Unread category breakdown of the **500 newest** unread, labeled as a sample. Its headline figure used to be the length of one capped `maxResults: 500` page, so any mailbox past 500 unread reported exactly 500 as a hard number (a true 147,970 the day it was fixed). `--total` adds the exact mailbox count, opt-in because it pages the whole match set — 82s at 148k unread, the same tradeoff as `list-unlabeled-unread.mjs --all`. **Not a `report-messages.mjs` preset**: it files each message under the **first** label it carries from an ordered priority list, and `--stats` reports per-label totals — neither is a query-and-print. The taxonomy is `config/tracked-labels.mjs`. `--count` and `--verify` are **gone**: `report-messages.mjs --total --count "is:unread"` is exact where `--count` read `labels.get` (1,313 reported against a true 13), and `audit-label-drift.mjs --query "<sender>" --expect "<label>"` draws the expectation from the config, where the two hardcoded spot-checks drifted from it. `--schema` is now `audit-schema-markup.mjs`
...
- `modify-messages.mjs (--label "<name>" | --query "<gmail-query>") [--unread] [--before YYYY/MM/DD] [--exclude-label "<label>"] [--add "<label>"] [--remove "<label>"] [--yes]` — General select-and-relabel: resolves labels, pages the selection, batch-modifies it. **Previews and changes nothing without `--yes`.** Selects by `labelIds` rather than `label:"…"` (a label name is unsafe search input — see [README.md#known-issues](README.md#known-issues)), and every named label must already exist, so a typo fails before the paging spend. System names (`UNREAD`/`INBOX`/`SPAM`) resolve through the same lookup as user labels. Seven presets delegate here: `mark-spam.mjs`, `mark-old-label-read.mjs`, `relabel-messages.mjs`, `archive-old-emails.mjs`, `mark-forums-read.mjs`, `mark-read.mjs` and `strip-label.mjs`. `previewHint` lets a preset gated on `--dry-run` avoid a preview that names `--yes`. Pass `quiet: true` when a preset loops and prints its own per-iteration summary.
  `--exclude-label` subtracts a protected set **by label id**, which is why it exists at all —
  `-label:"Keep Important"` depends on how Gmail search tokenizes a name containing a space.
  It absorbed `bulk-archive-unread.mjs`, whose whole job is now
  `--query "is:unread in:inbox" --exclude-label "Keep Important" --remove INBOX --yes`
  (still resumable: an archived message drops out of the query). It also passes
  `batchModifyMessages`' `onSkipped`, so a message Gmail persistently refuses to modify is
  bisected out and **warned about** rather than ending the run — the reported `modified` is
  then honestly short of `matched`
- `relabel-messages.mjs --query "<gmail-query>" [--add "<label>"] [--remove "<label>"] [--yes]` — Move a query's matches between user labels; both labels must already exist (fails fast rather than creating one from a typo). A `modify-messages.mjs` preset, so it pages to exhaustion — it used to cap at 100 and print a success line on a larger set. **Previews without `--yes`**, because unbounded is a far bigger action than the cap it replaced. Use `strip-label.mjs` for bulk removal
- `strip-label.mjs --label "X" [--query "<gmail-query>"] [--dry-run]` — Remove a label from every message carrying it; `--query` narrows that to one sender's copies. A `modify-messages.mjs` preset, so selection goes through `labelIds` and never a `label:"…"` query — this script was the standing example of that failure, reporting `Stripped 0` against a full label. The re-query-the-first-page loop it used to need is gone too: `modify-messages` collects every id before modifying anything, so no page token is held across a mutation. Gated on `--dry-run` rather than `--yes`, so it passes its own `previewHint`. **Check what remains labeled before stripping** — stripping `Product Updates` off AlphaSignal mail would have orphaned 215 of 321 messages, because only the rest also carried `Newsletters`; backfill the correct label first (`labelAllMatching()` in `lib/gmail-tag-utils.mjs` pages properly)
- `merge-label.mjs --from "A" --into "B" [--delete-source] [--dry-run]` — Add B to every message carrying A, then optionally delete A. Selects by `labelIds`, **never a `label:"…"` query**: a label name is unsafe search input — parens return 0 matches, and `&`/spaces can tokenize into a *sibling* label. Both fail silently, so a query-based merge can operate on the wrong set and still print success; `Food` → `Food & Delivery` was exactly that prefix collision. Pages with `pageToken` (safe because adding a label leaves the source set unchanged), deletes via one `labels.delete` rather than a bulk strip so there is no partial-detach window, and refuses to delete unless the target ends up holding at least as many messages as the source did
- `protect-important-inbox.mjs` — Keep a hand-picked set of senders in the inbox under `Keep Important`. The --billing modes that used to live here are `route-billing-mail.mjs`; the two shared only `cappedSweep` (now in `lib/gmail-batch-utils.mjs`): one names four senders and keeps their mail, the other matches billing keywords and archives most of what it finds. **Not a `create-org-tags.mjs`-shaped tag set** despite looking like one (one label, flat `{name, query}` entries, no archive) — one entry's subject clause used to be OR'd onto its sender clause with no sender constraint of its own, matching 410 unrelated messages mailbox-wide and mislabeling 134 of them (fixed 2026-08-26 by AND-ing it with a sender clause instead; see [README.md#known-issues](README.md#known-issues)). Every entry is sender-scoped now, but `cappedSweep`'s cap stays rather than switching to `applyTagSet()`'s unbounded backfill — the failure mode is exactly what a future subject-only entry would reintroduce
- `route-billing-mail.mjs [--update | --apply-only]` — Keep urgent billing alerts (late fee/overdue/missed payment) in the inbox, archive the routine rest. Was `protect-important-inbox.mjs --billing`; the two shared only `cappedSweep`. **Every query here matches subject words, not senders**, so each sweep is capped at `SUBJECT_SWEEP_CAP` and says when it truncated. Its four rules are named constants — `RATE_LIMIT_KEEP`/`RATE_LIMIT_REST`, `URGENT_KEEP`/`REGULAR` — each pair a `subject:` test and its exact `-subject:` complement, because both halves used to be spelled out independently and disagreed: **the urgent terms were unquoted inside an OR group** (so `(late fee OR overdue OR "missed payment")` collapsed to `late AND fee`, matching 8 where the quoted form matches 100), and **the negation was scoped to the whole message rather than the subject** (149 billing messages matched neither half). See [README.md#known-issues](README.md#known-issues)
- `filter-events-by-date.mjs` — Filtering & organization
- `mark-read.mjs [--archived-only]` — Mark unread mail under the six routine categories as read. A `modify-messages.mjs` preset, applied per label, so selection goes through `labelIds` and a label name with `&` or parens cannot silently select nothing. Its `--past-events` mode is **gone** — `mark-past-events-read.mjs` does that job and chunks, pages and retries, where the flag loaded 500 full bodies in one `Promise.all` and issued a single modify at the end
- `mark-forums-read.mjs` — Mark Forums mail older than 5 days as read; a `modify-messages.mjs` preset that applies immediately (one label, and removing UNREAD is idempotent)
- `archive-old-emails.mjs (--label "X" | --query "<gmail-query>") [--yes]` — Archive mail older than 7 days; `--query` covers senders that should sit in the inbox briefly, since Gmail filters run only on arrival and can't express age. A `modify-messages.mjs` preset: **previews without `--yes`**, and the age test is now Gmail's own `before:` rather than a `messages.get` per candidate to read `internalDate` client-side
...
- `gmail-message-utils.mjs` — Message header/body extraction, `decodeBase64Payload()`, `countMessagesMatching()` (exact paginated query counts), `fetchMessageHeaders()` (format=metadata: subject/from/date/labels), `fetchFullMessages()` (format=full: same fields plus `bodyText`/`payload`) — both retried and concurrency-bounded, unlike the raw `gmail.users.messages.list`/`.get()` calls four scripts used to hand-roll (one had no retry and no per-call catch, so a single failed fetch aborted the whole run; two fetched one message at a time with no concurrency at all)
- `gmail-query-tokens.mjs` — Sender-token parsing for Gmail *search queries* and filter *criteria* (not `From` headers — that's `email-utils.mjs`): `fromTokens()` (handles both `from:a OR from:b` and grouped `from:(a OR b)`), `criteriaTokens()`, `tokensOverlap()` (bidirectional domain-suffix match, used to decide whether two ROUTING RULES name the same sender). Pulled out of `audit-label-drift.mjs` — a CLI entrypoint — once `audit-org-tag-coverage.mjs` needed `fromTokens()` too; importing a function from a CLI script drags in that script's own config loading. `tokensOverlap()`'s permissiveness (one address on a shared platform overlaps with the bare domain) is wrong for `audit-org-tag-coverage.mjs`'s own `isCovered()`, which stays separate — see the comment on `isCovered()`
...
- `email-analyzer.mjs` / `email-utils.mjs` — Email parsing, categorization helpers. **All sender
  ...
  `audit-org-tag-coverage.mjs` and the former `sublabel-services.mjs`, with two scripts
  importing them *from a CLI entrypoint*; don't re-add a local regex variant
