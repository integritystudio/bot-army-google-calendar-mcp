/**
 * Category routing config: the single source of truth for which senders get which
 * label on arrival, and how existing mail is backfilled into it.
 *
 * Data only — no Gmail client, no CLI. create-filters.mjs consumes this to sync live
 * filters; audit-label-drift.mjs consumes it to check what the mail actually carries.
 * Keeping it out of the CLI entrypoint means an auditor reading CATEGORIES does not
 * load googleapis or the filter-sync engine behind it.
 */
import {
  DEFAULT_MAX_RESULTS,
  LABEL_SENTRY,
 ...
 *   entries       — array of { name, query } for createGmailFilter; an entry may set
