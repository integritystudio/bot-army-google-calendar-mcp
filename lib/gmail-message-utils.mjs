...
import { getHeader, decodeBase64Payload, normalizeWhitespace } from './email-utils.mjs';
...
/**
 * Fetch full messages (format=full — the whole payload, body included), retried and
 * concurrency-bounded like fetchMessageHeaders above.
 *
 * Three call sites hand-rolled this: one already used mapWithConcurrency but with no
 * retry, so a single failed fetch rejected the whole batch (mapWithConcurrency's workers
 * are not individually caught); the other two fetched sequentially, one message at a
 * time, with no concurrency at all. `bodyText` is precomputed via extractBodyText so
 * callers that only want plain text never touch `payload`; callers that need the raw
 * structure (schema.org markup extraction, which reads HTML a different way than
 * extractBodyText does) still have it.
 *
 * @param {Object} gmail - Gmail API client
 * @param {string[]} ids - Message IDs
 * @returns {Promise<Array<{id: string, subject: string, from: string, date: string,
 *   internalDate: string, labelIds: string[], bodyText: string, payload: Object}>>}
 */
export async function fetchFullMessages(gmail, ids) {
  let failed = 0;
  const fullMsgs = await mapWithConcurrency(ids, id =>
    withRetry(() => gmail.users.messages.get({
      userId: USER_ID,
  ...
  if (failed > 0) {
    console.warn(`fetchFullMessages: ${failed} of ${ids.length} messages could not be fetched`);
  }
  return fullMsgs.filter(Boolean).map(msg => {
