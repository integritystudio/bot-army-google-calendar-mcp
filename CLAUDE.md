...
- `route-billing-mail.mjs [--update | --apply-only]` — Keep urgent billing alerts (late fee/overdue/missed payment) in the inbox, archive the routine rest. Was `protect-important-inbox.mjs --billing`; the two shared only `cappedSweep`. **Every query here matches subject words, not senders**, so each sweep is capped at `SUBJECT_SWEEP_CAP` and says when it truncated
