...
export const ToolSchemas = {
  ...
  'gmail-search-messages': z.object({
    query: z.string().describe(
      "Gmail search query (e.g., 'is:unread', 'from:example@gmail.com', 'subject:hello', 'is:unread AND from:work@company.com')"
    ),
    maxResults: z.number().int().min(1).max(100).optional().describe(
      "Maximum number of messages to return (1-100, default: 10)"
    ),
    pageToken: z.string().optional().describe(
      "Token for pagination to get next batch of results"
    )
  }),

  'gmail-get-profile': z.object({}),

  'gmail-modify-messages': z.object({
    messageIds: z.array(z.string()).min(1).describe(
      "Array of message IDs to modify (at least 1 required)"
    ),
    action: z.enum(["markRead", "markUnread", "archive", "delete", "addLabel", "removeLabel"]).describe(
      "Action to perform: markRead, markUnread, archive, delete, addLabel, removeLabel"
    ),
    labelId: z.string().optional().describe(
      "Required for addLabel and removeLabel actions. Gmail label ID to add or remove."
    )
  }),

  'gmail-create-label': z.object({
    name: z.string().min(1).describe("Name of the new label"),
    labelListVisibility: z.enum(["labelShow", "labelHide"]).optional().describe(
      "Visibility in label list (labelShow or labelHide, default: labelShow)"
    ),
    messageListVisibility: z.enum(["show", "hide"]).optional().describe(
      "Visibility in message list (show or hide, default: show)"
    )
  }),

  'gmail-create-filter': z.object({
    criteria: z.object({
      ...
      query: z.string().optional().describe("Gmail search query for filter matching"),
    ...
    }).refine(obj => Object.keys(obj).length > 0, "At least one criteria is required"),
    action: z.object({
      addLabelIds: z.array(z.string()).optional().describe("Label IDs to add"),
      removeLabelIds: z.array(z.string()).optional().describe("Label IDs to remove"),
      archive: z.boolean().optional().describe("Auto-archive matching emails"),
      markAsRead: z.boolean().optional().describe("Auto-mark as read"),
      markAsSpam: z.boolean().optional().describe("Auto-mark as spam"),
      markAsTrash: z.boolean().optional().describe("Auto-move to trash"),
      forward: z.string().optional().describe("Forward to email address"),
      neverMarkAsSpam: z.boolean().optional().describe("Never mark as spam")
    }).refine(obj => Object.keys(obj).length > 0, "At least one action is required")
  }),

  'gmail-apply-filters': z.object({
...
export type GmailSearchInput = ToolInputs['gmail-search-messages'];
export type GmailGetProfileInput = ToolInputs['gmail-get-profile'];
export type GmailModifyInput = ToolInputs['gmail-modify-messages'];
export type GmailCreateLabelInput = ToolInputs['gmail-create-label'];
export type GmailCreateFilterInput = ToolInputs['gmail-create-filter'];
export type GmailApplyFiltersInput = ToolInputs['gmail-apply-filters'];
...
export class ToolRegistry {
  ...
  private static tools: ToolDefinition[] = [
    ...
    },
    {
      ...
      description: "Search Gmail messages by query (e.g., 'is:unread', 'from:email@example.com')",
      schema: ToolSchemas['gmail-search-messages'],
      handler: GmailSearchHandler
    },
    {
      name: "gmail-get-profile",
      description: "Get Gmail profile information including message and thread counts",
      schema: ToolSchemas['gmail-get-profile'],
      handler: GmailGetProfileHandler
    },
    {
      name: "gmail-modify-messages",
      description: "Modify Gmail messages: mark read/unread, archive, delete, add/remove labels",
      schema: ToolSchemas['gmail-modify-messages'],
      handler: GmailModifyHandler
    },
    {
      name: "gmail-create-label",
      description: "Create a new Gmail label for organizing emails",
      schema: ToolSchemas['gmail-create-label'],
      handler: GmailCreateLabelHandler
    },
    {
      name: "gmail-create-filter",
      description: "Create a Gmail filter to auto-organize emails (label, archive, delete, etc.)",
      schema: ToolSchemas['gmail-create-filter'],
      handler: GmailCreateFilterHandler
    },
    {
      name: "gmail-apply-filters",
      description: "Apply all existing Gmail filters to existing emails that match their criteria. Useful for retroactively applying filter rules to old emails.",
      schema: ToolSchemas['gmail-apply-filters'],
      handler: GmailApplyFiltersHandler
    }
  ];
