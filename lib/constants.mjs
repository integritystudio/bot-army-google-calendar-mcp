...
export const LABEL_EVENTS_RECRUITING = 'Events/Recruiting';
...
export const LABEL_COLD_OUTREACH = 'Cold Outreach';
...
/**
 * Sustained messages.get rate. Gmail allows 15,000 quota units per minute per user and a
 * messages.get costs 5, so 50/sec is the hard ceiling; stay under it to leave room for the
 * list and batchModify calls a bulk run interleaves.
 *
 * Bounding concurrency does NOT bound rate: 25 workers against a responsive mailbox issue
 * several times this, and once the per-minute units are gone every worker 429s at once.
 * withRetry gives up after four attempts, so a 12.5k-message run lost 7,291 fetches and
 * then threw before marking anything.
 */
export const GMAIL_FETCHES_PER_SECOND = 30;
