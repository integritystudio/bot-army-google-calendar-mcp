...
import { decodeBase64Payload } from './email-utils.mjs';

const SCHEMA_ORG_CONTEXT = /schema\.org/i;

const TRAVEL_RESERVATION_TYPES = new Set([
  'FlightReservation', 'LodgingReservation', 'RentalCarReservation',
...
export function categorizeBySchema(schemaObjects) {
  ...
  for (const type of types) {
    if (SCHEMA_CATEGORY_MAP[type]) { category = SCHEMA_CATEGORY_MAP[type]; break; }
  }
  ...
  for (const obj of schemaObjects) {
    ...
    } else if (TRAVEL_RESERVATION_TYPES.has(type)) {
...
/**
 * @param {Object} payload - Gmail message.data.payload
 * @returns {string} Base64url-decoded HTML or empty string
 */
