...
- `dump-messages.mjs [--max N] "<gmail-query>"` — TSV dump (date, from, subject) of messages matching any Gmail query; count goes to stderr so stdout pipes cleanly. A `report-messages.mjs` preset that sets `--total`
- `list-inbox.mjs` — Inbox grouped by Gmail's category tabs, unread marked `*`; a `report-messages.mjs` preset
