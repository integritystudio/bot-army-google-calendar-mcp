...
433); sender-matching regexes miss grouped `from:(a OR b)` queries and `from:"Display Name"`
...
- `create-country-tags.mjs [--filters-only] [--only <label>] [--countries a,b]` — Country/* sender-origin tags; label-only like `create-org-tags.mjs` (never archives). Seeded from ccTLD domains only — brands sending localized mail from a global domain can't be attributed and are left untagged. `COUNTRY_TAGS` is exported for `audit-label-drift.mjs`; note the three config sources name their entry array differently (`CATEGORIES.filters`, `ORG_TAGS.orgs`, `COUNTRY_TAGS.entries`), so anything iterating all three must map the field per source
