...
- `mark-read.mjs [--archived-only]`, `mark-forums-read.mjs`, `archive-old-emails.mjs (--label "X" | --query "<gmail-query>")` — Read/archive maintenance (7-day cutoff; `--query` covers senders that should sit in the inbox briefly, since Gmail filters run only on arrival and can't express age). `mark-read.mjs`'s `--past-events` mode is **gone** — `mark-past-events-read.mjs` does that job and chunks, pages and retries
